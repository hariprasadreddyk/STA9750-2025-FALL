...
```{r}
#| code-fold: true
#| code-summary: "Download American Community Survey (ACS) data (2009–2023) for income, rent, population, and households"
#| message: false
#| warning: false
...
```{r}
#| code-fold: true
#| code-summary: "Download U.S. Census Building Permits Survey data (2009–2023) for new housing units by CBSA"
#| message: false
#| warning: false
...
```{r}
#| code-fold: true
#| code-summary: "Download and compile BLS QCEW annual employment and wage data (2009–2023) for all CBSAs"
#| message: false
#| warning: false
...
#| code-summary: "Top 5 CBSAs by total new housing units permitted (2010–2019)"
#| message: false
#| warning: false
...
Combined, these Houston CBSAs account for roughly **`r scales::comma(top5_cbsa_permits[[ "Total Permitted Units (2010–2019)" ]][1])`** new permitted housing units. A clear indicator of sustained regional growth and a strong pro-development environment.  
...
The **New York–Northern New Jersey–Long Island, NY–NJ–PA** CBSA last had the most data scientists (NAICS 5182) in **`r nyc_last_lead$YEAR`**.
...
This relationship reflects how local affordability challenges emerge: while wealthier regions can support higher rents, lower-income metros remain constrained by limited wage growth, influencing overall housing accessibility.
...
This visualization shows that **healthcare employment** scales closely with **total employment** across U.S. metro areas from 2009–2023.
...
## 3.3 Evolution of Average Household Size (Spaghetti + Highlight NYC & LA)
```{r}
...
cat("Housing growth data prepared.\n")
cat(glue("Years covered: {min(housing_growth_data$year)} to {max(housing_growth_data$year)}\n"))
cat(glue("Number of CBSAs: {length(unique(housing_growth_data$CBSA))}\n"))
```
...
cat(glue("Instantaneous Growth Baseline (2014 median): {round(baseline_instantaneous, 2)} permits per 1,000 residents\n"))
```
...
cat(glue("Rate-Based Growth Baseline (2014 median): {round(baseline_rate, 1)}% (permits relative to 5-yr pop growth)\n"))
```
...
**Key Findings from Composite Housing Growth Analysis:**

**YIMBY Champions - Top Performers:**
- **Small-to-Mid Markets Leading:** Springfield, OH; Dalton, GA; and Brunswick-St. Simons, GA show that smaller metros can achieve exceptional pro-housing outcomes
- **Major Metro Success:** Miami-Fort Lauderdale ranks 8th overall, demonstrating that large metros can maintain strong housing production
- **Southern Strategy:** Southern metros dominate the top 15, suggesting regional policy differences favoring development

**Growth Constrained Markets - Bottom Performers:**
- **Declining Markets:** Morgantown, WV (6.1 composite) and Enid, OK (20.7) struggle with both low absolute construction and poor growth response
- **Supply Bottlenecks:** Atlantic City, NJ (17.6) and Longview, TX (21.2) show inadequate supply response to population pressures
- **Appalachian/Great Plains Pattern:** These regions disproportionately represented among constrained markets

**Policy Implications:**

Markets with high composite scores demonstrate that pro-housing policies can succeed across different contexts—from fast-growing Sun Belt metros to slower-growth Midwest markets. The gap between leaders (3,329.7) and laggards (6.1) suggests significant room for policy intervention to improve housing supply responsiveness.
...
cat(glue("Total metros in analysis: {nrow(yimby_success)}\n"))
cat(glue("YIMBY Success Stories (all 4 criteria): {sum(yimby_success$yimby_success)}\n"))
cat(glue("3 of 4 criteria met: {sum(yimby_success$yimby_score == 3)}\n"))
```

**YIMBY Identification Criteria:**

We identify YIMBY success stories as metros meeting all four conditions:<\br>
1. ✅ **High Initial Burden:** Above-median rent burden in 2014-2016 <\br>
2. ✅ **Improved Affordability:** Decreased rent burden from early to recent period  <\br>
3. ✅ **Growing Markets:** Positive population growth (not declining cities) <\br>
4. ✅ **Strong Supply Response:** Above-median housing growth score <\br>

**Results:** 30 metros (out of 308 analyzed) meet all four YIMBY success criteria.
...
All five top-performing metros showed sustained rent burden reduction over the 2014-2023 period:

- **Bangor, ME:** Most consistent decline (104 → 95), demonstrating steady policy commitment
- **Brownsville-Harlingen, TX:** Net improvement despite volatility (125 → 107)
- **Redding, CA:** Dramatic improvement after 2018 peak (129 → 110)

These trajectories prove that YIMBY success requires sustained multi-year commitment to housing production, not one-time policy changes.

---

**Conclusion:**

These 30 YIMBY success stories demonstrate that building housing can improve affordability even in growing, desirable markets. Success spans diverse contexts—from small Southern metros to coastal communities—proving that pro-housing policies work when sustained over time.
...
cat(glue("Primary Sponsor (YIMBY Success): {primary_sponsor_metro$NAME}\n"))
cat(glue("  - Population: {scales::comma(primary_sponsor_metro$population)}\n"))
cat(glue("  - Rent Burden Change: {round(primary_sponsor_metro$rent_burden_change, 1)}\n"))
cat(glue("  - Housing Growth Score: {round(primary_sponsor_metro$avg_composite_score, 1)}\n\n"))

cat(glue("Co-Sponsor (NIMBY Challenge): {nimby_metro$NAME}\n"))
cat(glue("  - Population: {scales::comma(nimby_metro$population)}\n"))
cat(glue("  - Rent Burden Change: +{round(nimby_metro$rent_burden_change, 1)}\n"))
cat(glue("  - Housing Growth Score: {round(nimby_metro$avg_composite_score, 1)}\n"))
```
...
cat("Formatted FIPS codes:\n")
cat(glue("Albuquerque: {albuquerque_fips}\n"))
cat(glue("Atlanta: {atlanta_fips}\n"))
...
cat("\nHealthcare and Construction Sector Comparison:\n")
print(key_sectors)
...
cat("\nTop Sectors by Employment (Albuquerque):\n")
all_sectors |>
  dplyr::filter(metro == "Albuquerque") |>
  dplyr::slice_head(n = 5) |>
  print()

cat("\nTop Sectors by Employment (Atlanta):\n")
all_sectors |>
  dplyr::filter(metro == "Atlanta") |>
  dplyr::slice_head(n = 5) |>
  print()
...
cat("\nRent Burden Comparison (2023):\n")
print(rent_comparison)

# Calculate savings for healthcare workers
cat("\nAffordability Impact Calculation:\n")
cat(glue("If Atlanta healthcare workers ({key_sectors$employment_Atlanta[key_sectors$sector_name == 'Healthcare & Social Services']} workers) "))
cat(glue("had Albuquerque's rent burden, they would save:\n"))
cat(glue("Current: {rent_comparison$rent_as_pct_income[rent_comparison$metro == 'Atlanta'] |> round(1)}% of income on rent\n"))
cat(glue("Target: {rent_comparison$rent_as_pct_income[rent_comparison$metro == 'Albuquerque'] |> round(1)}% of income on rent\n"))
```
...
- Achieved a **Housing Growth Score of 203.1**—double the national median
...
- **Rent burden increased by 8.6 points** since 2014—among the worst deteriorations nationally
- Housing Growth Score of 165.9—insufficient to meet demand
...
- **Why They Win:** 
  ...
  - Lower housing costs mean nurses, EMTs, and social workers can afford to live where they work—critical for healthcare workforce retention
...
- Data already collected by Census Bureau and BLS—**no new bureaucracy**
...
**Supporting data, detailed methodology, and economic impact analysis available upon request.**
